      ...
      <div className="mt-6 p-4">
        <div className="bg-cyan-50">
          <div className="tab-list flex align-top justify-center">
            <button
              className={`tab btn btn-outline bg-cyan-200 mr-2  p-2 ${activeTab === 0 ? "active" : ""}`}
              onClick={() => handleTabClick(0)}
            >
              Avengers
            </button>
            <button
              className={`tab btn btn-outline bg-cyan-200 mr-2 p-2 ${activeTab === 1 ? "active" : ""}`}
              onClick={() => handleTabClick(1)}
            >
              Transformers
            </button>
            <button
              className={`tab btn btn-outline bg-cyan-200 p-2 ${activeTab === 2 ? "active" : ""}`}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {categoryData.map((item, index) => (
                <div key={index} className="p-4 border rounded-lg">
                  ...
                  />
                  <h3 className="text-xl text-center py-2 font-semibold text-cyan-600 mb-2">{item.name}</h3>
                  <p className="text-cyan-700 font-bold text-center  py-2 mb-2">Price :{item.price} $</p>
                      <div className="flex justify-center gap-8">
                      <div className="flex justify-normal items-center mb-2">
                    <span className="text-yellow-500 text-xl  hover:text-cyan-500 font-bold"> Ratings:{item.rating}</span>
                    <svg
                  ...
                  <Link to={`/detail/${item._id}`} className="btn btn-outline bg-cyan-200">
          View Detail
        </Link>
                 </div>
                </div>
